...
class SiteTests(unittest.TestCase):
    ...
    def test_drill_visuals_are_accessible_and_motion_safe(self):
        ...
        for path in sorted((ROOT/'assets/drill').glob('*.svg')):
            ...
            self.assertNotRegex(svg,r'@keyframes\s+[\w-]+-settle\s*\{[^}]*transform:',path)
            reduced=re.search(r'@media \(prefers-reduced-motion: reduce\)\{(.*?)\}\}',svg,re.S).group(1)
            self.assertNotRegex(reduced,r'\.motion-layer[^}]*transform\s*:\s*none',path)
            cadets=root.findall(f'.//{ns}g[@data-cadet]')
            self.assertGreaterEqual(len(cadets),4,path)
            self.assertTrue(all('scale(' in cadet.get('transform','') for cadet in cadets),path)
